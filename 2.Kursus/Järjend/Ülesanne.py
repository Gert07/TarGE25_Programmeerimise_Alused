...
def add_element_in_position(elements: list, new_element: any, position: int) -> list:
    ...
    """
    elements[position:position] = [new_element]
    print(elements)
...
if __name__ == "__main__":
    create_list_of_two_elements(3, 2)
    create_list_with_edge_elements([1, 2, 3])
    get_middle_element_of_list([1, 2, 3, 4, 5])
    get_middle_part_of_list(["a", "b", "c", "d", "e", "f"])
    create_new_list_with_added_number([1, 2, 3], 4)
    swap_edge_elements(["a", "b", "c", "d", "e", "f"])
    add_element_in_position([1, 2, 3], 5, 3)
    get_repeated_list([1, 2, 3], 7)
    remove_first_element_from_list([7])
    reverse_list(["a", "b", "c"])
