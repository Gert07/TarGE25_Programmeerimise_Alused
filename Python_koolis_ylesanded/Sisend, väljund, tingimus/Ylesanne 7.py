...
def get_product_sum(id_code: str, level_one_weigths: list[int]) -> int:
    result_sum = 0
...
def get_checksum(id_code: str) -> int:
    level_one_weigths = [1,2,3,4,5,6,7,8,9,1]
    level_two_weights = [3,4,5,6,7,8,9,1,2,3]
    product = get_product_sum(id_code, level_one_weigths)
    remainder = product_sum % 11
    if remainder < 10:
        return remainder
    product = get_product_sum(id_code, level_two_weigths)
    remainder = product_sum % 11
    if remainder < 10:
        return remainder
    return 0
...
def decode_id_code(id_code: str) -> None:
    if is_valid(id_code):
        ...
        print(f"Oled {gender} soost. Sündinud: {get_date_string(birth_date)}.")
    else:
        print(f"Viga sisestatud isikukoodis: {id_code}")


if __name__ == "__main__":
    isikukood = "39902201513"
