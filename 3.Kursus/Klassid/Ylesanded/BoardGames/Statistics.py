...
class Statistics:
    ...
    def get(self, path:str):
        ...
        "/game/{name}/most-frequent-loser" - tagastab mängija string, kelle kaotuse protsent (protsent kordadest, kui mängija jäi viimasele kohale)
            mängus nimega game_name on suurim (seda funktsiooni kutsutakse vaid points või places mängu korral)
        "/game/{name}/record-holder" - tagastab mängija (string), kes on mängus nimega game_name saanud enim punkte (ühe mängu jooksul),
            viigi korral tagastada see, kes selle tulemuse esimesena saavutas (seda funktsiooni kutsutakse vaid points mängu korral)"""
        if path == "/players":
            return list(map(lambda p: p.get_name(), self.__players))
        elif path == "/games":
            return list(map(lambda g: g.get_name(), self.__game))
        return None